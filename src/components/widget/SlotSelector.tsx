...
export function SlotSelector({ business, service, staff, date, onSelect, onBack }) {
    ...
    const fetchSlots = async () => {
      ...
      try {
        const res = await fetch(
          `/api/slots?business_id=${business.id}&service_id=${service.id}&date=${date}`
        )
  ...
  const availableSlots = slots.flatMap(s =>
    s.slots
        ...
        <div>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {availableSlots.map((slot, i) => (
              <button
                key={i}
                onClick={() => onSelect(slot, { id: slot.staff_id, name: slot.staff_name })}
              ...
              >
                ...
                <p className="text-xs text-gray-400 mt-0.5">{slot.staff_name}</p>
              </button>
            ))}
          </div>
        </div>
