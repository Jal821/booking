...
export default function ServicesPage() {
  ...
  const [form, setForm] = useState({
    ...
    cleanup_minutes: 10, price: 0, currency: 'EUR', color: '#6366f1',
  })

  const fetchServices = async () => {
    const res = await fetch(`/api/services?business_id=${BUSINESS_ID}`)
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-6 mb-6 grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <input required className="w-full border rounded-lg px-3 py-2 text-sm" value={form.name}
              onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Manikura" />
          </div>
          <div className="col-span-2">
          ...
          <div>
            ...
            <input required type="number" className="w-full border rounded-lg px-3 py-2 text-sm" value={form.duration_minutes}
              onChange={e => setForm({ ...form, duration_minutes: Number(e.target.value) })} />
          </div>
          <div>
            <p className="block text-sm font-medium text-gray-700 mb-1">Cleanup time (minutes)</p>
            <input type="number" className="w-full border rounded-lg px-3 py-2 text-sm" value={form.cleanup_minutes}
              onChange={e => setForm({ ...form, cleanup_minutes: Number(e.target.value) })} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="color" className="w-full border rounded-lg px-3 py-2 h-10" value={form.color}
          ...
          <div className="col-span-2 flex gap-3 justify-end">
            ...
            <button type="submit"
              className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Save Service</button>
          </div>
        </form>
      )}
          ...
          {services.map((service) => (
            <div key={service.id} className="bg-white rounded-xl shadow-sm p-5">
              ...
              <div className="flex gap-4 text-sm text-gray-600 mb-4">
                ...
                <span>{service.cleanup_minutes} min cleanup</span>
                <span>{service.price} {service.currency}</span>
              </div>
              <button onClick={() => handleDelete(service.id)}
                className="text-red-500 text-sm hover:text-red-700">Deactivate</button>
            </div>
          ))}
        </div>
      )}
